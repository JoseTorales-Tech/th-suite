...
const Btn = ({children,onClick,color=C.accent,outline=false,style={}}) => <button onClick={onClick} style={{background:outline?'transparent':color,border:`2px solid ${color}`,color:outline?color:'#fff',borderRadius:8,padding:'8px 18px',fontWeight:700,fontSize:13,cursor:'pointer',...style}}>{children}</button>
...
const Modal = ({title,children,onClose}) => <div style={{position:'fixed',inset:0,background:'#000a',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:18,padding:28,minWidth:420,maxWidth:560,width:'90%',maxHeight:'90vh',overflowY:'auto'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h3 style={{color:C.text,margin:0,fontSize:18,fontWeight:800}}>{title}</h3><button onClick={onClose} style={{background:'none',border:'none',color:C.muted,fontSize:22,cursor:'pointer'}}>×</button></div>{children}</div></div>
    ...
    <div>
        ...
        <div style={{display:'flex',gap:12,overflowX:'auto',paddingBottom:12}}>
          {STAGES.filter(s=>s!=='Rechazado').map(stage=>{
            ...
            return <div key={stage} style={{minWidth:190,flex:'0 0 190px'}}>
              ...
              {cols.map(c=>(
                <div key={c.id} onClick={()=>setDetail(c)} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:12,marginBottom:8,cursor:'pointer'}}>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    {c.score>0?<Badge label={`${c.score}%`} color={c.score>=80?C.success:C.warning}/>:<span/>}
        ...
        <Card>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>{candidates.map(c=><tr key={c.id} style={{borderBottom:`1px solid ${C.border}22`}}>
              ...
              <td style={{padding:12}}><Btn onClick={()=>setDetail(c)} style={{padding:'5px 12px',fontSize:12}}>Ver</Btn></td>
            </tr>)}</tbody>
          </table>
        </Card>
      )}
      {modal==='new'&&<Modal title="Nuevo Candidato" onClose={()=>setModal(null)}>
        ...
        <div style={{display:'flex',gap:10,justifyContent:'flex-end',marginTop:10}}>
          ...
          <Btn onClick={add} style={{opacity:loading?0.6:1}}>{loading?'Guardando...':'Agregar'}</Btn>
        </div>
      </Modal>}
      {detail&&<Modal title="Detalle Candidato" onClose={()=>setDetail(null)}>
        ...
        <div style={{marginBottom:12}}><div style={{color:C.muted,fontSize:11,fontWeight:700,marginBottom:8}}>MOVER A ETAPA</div>
...
function Evaluation({evaluations,setEvaluations}) {
  const [modal,setModal]=useState(null)
        ...
        <div style={{marginLeft:220,flex:1,padding:32,minHeight:'100vh'}}>
          {loading?(
            ...
            <>
              ...
              {tab==='evaluation'&&<Evaluation evaluations={evaluations} setEvaluations={setEvaluations}/>}
